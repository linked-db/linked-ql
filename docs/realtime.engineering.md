...
A huge problem for reactive data systems is duplication of work across overlapping subscriptions. Traditional realtime systems fulfill each subscription in isolation — each maintaining its own feed, evaluating its own filters, and reconstructing the same result sets as a peer. The model is conceptually simple but operationally expensive. A simple variation on a new subscription — an added predicate or ordering clause — is enough to spawn an entirely new reactive process.
...
#### a) Linear Fan-Out (The Common Case)

Let:

* {% katex inline %}S{% endkatex %} = number of active subscriptions (distinct live queries),
* {% katex inline %}C_q{% endkatex %} = average CPU cost to the server to re-execute one subscribed query,
* {% katex inline %}R{% endkatex %} = average network cost (bytes or roundtrip time) to deliver one query result to a client,
* {% katex inline %}E{% endkatex %} = number of upstream DB change events in a time window.
...
* Suppose {% katex inline %}S = 1{,}000{% endkatex %} subscriptions.
* Suppose each request cost {% katex inline %}C_q = 10{% endkatex %} ms of CPU work.
* Suppose {% katex inline %}R = 50{% endkatex %} ms of network/latency-equivalent cost.

Then for a single DB event:

* {% katex inline %}W_{\text{linear}} = 1{,}000 \times 10\ \text{ms} = 10{,}000\ \text{ms}{% endkatex %} of CPU time total (10 seconds).
* {% katex inline %}N_{\text{linear}} = 1{,}000 \times 50\ \text{ms} = 50{,}000\ \text{ms}{% endkatex %} of accumulated network latency-equivalent (50 seconds).
...
#### b) Cascading / Combinatorial Blow-Up (Worst-Case)
...
* {% katex inline %}b{% endkatex %} = branching factor (average number of new DB events caused by handling a single event),
* {% katex inline %}d{% endkatex %} = depth (number of propagation rounds you consider).

Number of events after {% katex inline %}d{% endkatex %} rounds:
{% katex %}
E_{\text{total}}(d) \;=\; \sum_{i=0}^{d} b^i \;=\; \frac{b^{d+1}-1}{b-1}\quad\text{(for }b\neq1\text{)}.
{% endkatex %}

Total query executions across all rounds (each event re-triggers all {% katex inline %}S{% endkatex %} subscriptions):
{% katex %}
...
* {% katex inline %}S = 1{,}000{% endkatex %},
* {% katex inline %}C_q = 10{% endkatex %} ms,
* {% katex inline %}b = 3{% endkatex %} (each handling step creates three further DB events on average),
* {% katex inline %}d = 3{% endkatex %} rounds.

Step-by-step:

1. {% katex inline %}b^{d+1} = 3^{4} = 81.{% endkatex %}
2. {% katex inline %}E_{\text{total}}(3) = (81 - 1) / (3 - 1) = 80 / 2 = 40{% endkatex %} total events.
3. {% katex inline %}W_{\text{cascade}} = 1{,}000 \times 10\ \text{ms} \times 40 = 400{,}000\ \text{ms}{% endkatex %} total CPU-equivalent (400 seconds).
...
* {% katex inline %}r{% endkatex %} = fraction of subscriptions that can be **served from shared parent windows** {% katex inline %}0 ≤ (r) ≤ 1{% endkatex %}.
* Effective independent subscriptions {% katex inline %}S_{\text{eff}} = S \times (1 - r){% endkatex %}.
...
* {% katex inline %}S = 1{,}000{% endkatex %}, {% katex inline %}C_q = 10{% endkatex %} ms.
* If {% katex inline %}r = 0.8{% endkatex %} (80% of subscriptions are subwindows that inherit), then:

  * {% katex inline %}S_{\text{eff}} = 1{,}000 \times 0.2 = 200{% endkatex %}.
  * {% katex inline %}W_{\text{linear, reused}} = 200 \times 10\ \text{ms} = 2{,}000\ \text{ms}{% endkatex %} (2 seconds) instead of 10 seconds — a 5× reduction.

If inheritance also reduces the branching factor {% katex inline %}b{% endkatex %} in cascades (because shared computation closes loops), the exponential/cascading costs collapse even faster.
...
### 1.3 Query Windows and Canonical Frames
...
### 1.5 Expression Canonicalization
...
### 1.7 SSR Mode and Inline Evaluation
...
### 1.9 Efficiency and Granularity
...
| ----------------------- | --------------- | --------------- | -------------- | -------- |
| Simple one-table filter | local           | 0               | {% katex inline %}O(events){% endkatex %}      | exact    |
| Simple join             | selective       | {% katex inline %}O(changed keys){% endkatex %} | {% katex inline %}O(rows){% endkatex %}        | exact    |
| Aggregate / window      | wholistic + ssr | {% katex inline %}O(result rows){% endkatex %}  | {% katex inline %}O(rows × expr){% endkatex %} | exact    |
| Subselect projection    | ssr             | {% katex inline %}O(result rows){% endkatex %}  | {% katex inline %}O(expr){% endkatex %}        | exact    |
...
### 2.1 Transactional Continuity

Let a database transaction {% katex inline %}T{% endkatex %} yield a set of low-level mutation events
...
The realtime engine guarantees that every layer {% katex inline %}S_i{% endkatex %} of the pipeline (driver → window → subwindow → consumer) preserves the *atomic boundary* of {% katex inline %}T{% endkatex %}:
...
### 2.3 Event Normalization and Row Tracking
...
Even when a row’s physical key changes between updates, continuity via {% katex inline %}h_{\text{track}}{% endkatex %} causes the system to coalesce rather than emit disjoint events.
Thus a flurry of internal row rewrites appears downstream as one clean, semantic `update`.

---

### 2.4 Logical Diff Construction
...
### 2.5 Join-Key Transition Semantics
...
For each left-hand key {% katex inline %}k_L{% endkatex %}:
...
maps pairs of join states {% katex inline %}s = (L,R){% endkatex %} to the minimal event preserving semantic continuity.
...
### 2.6 Transaction-Through Invariant

Once logical diffs are built, the engine enforces a strict *transaction-through* rule:

Every resulting batch {% katex inline %}D_T = \\{ d_1,…,d_m \\}{% endkatex %} remains atomic across propagation.
...
| **Row tracking**          | continuity preserved across key mutations                | `#normalizeEvents()` via {% katex inline %}h_{\text{track}}{% endkatex %}                           |
...
### 2.8 Subscription Interfaces and Mutation Handling
...
#### 2.8.1 Event Vocabulary and the Logic of Identity
...
Each record in an emission carries a **logical hash** {% katex inline %}h{% endkatex %} that expresses semantic identity:
...
#### 2.8.4 Observability and Its Universal Protocol
...
Each {% katex inline %}\delta_i{% endkatex %} is one of: set/update at a path, insert at an index, remove at an index, or swap between two indices. Applied in order, the batch yields the next state:

{% katex %}
S_{t+1} = S_t \oplus \Delta_t
{% endkatex %}

Where {% katex inline %}\oplus{% endkatex %} is the deterministic application defined by the Observer protocol. Because LinkedQL emits atomic batches, observers see no torn frames.
...
- Atomic visibility: a DB transaction corresponds to one observed batch {% katex inline %}\Delta_t{% endkatex %}.
- Minimality: diffs become just the operations that change; swaps become positional exchanges only.
- Composability: {% katex inline %}\Delta_t{% endkatex %} can be replayed across process or network boundaries without re-deriving from data.
...
#### 2.8.5 Incremental State Change

Formally, the evolution of any observable state {% katex inline %}S_t{% endkatex %} under incoming emissions is:

{% katex %}
S_{t+1} = S_t \oplus E_t
{% endkatex %}

where {% katex inline %}E_t{% endkatex %} is the next emission and {% katex inline %}\oplus{% endkatex %} is the deterministic application of that emission’s `result`, `diff`, or `swap`.
...
#### 2.8.6 Live Objects Contract (API and Invariants)
...
- Stable identity: each row carries a logical hash {% katex inline %}h{% endkatex %} computed as
...
3. **Minimal Change Algebra**  
   ...
   {% katex %}S_{t+1} \;=\; S_t \oplus D_T{% endkatex %}  
   where {% katex inline %}S_t{% endkatex %} is the current state, {% katex inline %}D_T{% endkatex %} is the logical diff derived from transaction {% katex inline %}T{% endkatex %}, and {% katex inline %}\oplus{% endkatex %} is the operator that applies it.  
...
The engineering of the LinkedQL Realtime Engine opens up new axes of exploration for reactivity over SQL — spanning realtime computation that remains **cost-bounded**, **operationally deterministic**, and **semantically coherent** across diverse database models: local, remote, or distributed.
...
3. **Semantic Continuity**
    The *live result model* represents each query as an ordinary object that simultaneously functions as a reactive data surface. State and change coexist within the same structure, eliminating the need for manual event handling, reconciliation logic, or synchronization layers.
