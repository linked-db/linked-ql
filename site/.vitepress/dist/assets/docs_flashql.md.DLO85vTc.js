import{_ as a,c as n,o as l,ag as e}from"./chunks/framework.B-XtCDNB.js";const d=JSON.parse('{"title":"FlashQL","description":"","frontmatter":{},"headers":[],"relativePath":"docs/flashql.md","filePath":"docs/flashql.md"}'),p={name:"docs/flashql.md"};function o(t,s,r,c,i,y){return l(),n("div",null,[...s[0]||(s[0]=[e(`<h1 id="flashql" tabindex="-1">FlashQL <a class="header-anchor" href="#flashql" aria-label="Permalink to &quot;FlashQL&quot;">​</a></h1><p><em>A full SQL engine for the local runtime, the edge, and the offline world.</em></p><p>FlashQL is LinkedQL’s embeddable database engine — a complete in-memory SQL runtime that runs anywhere JavaScript does: Node.js, browser, worker, or edge.</p><p>FlashQL can replace SQLite or PGLite across a variety of use cases — offering standard SQL semantics combined with LinkedQL’s extended capabilities, and native support for federation and synchronization.</p><p>Use FlashQL to:</p><ul><li>Run full SQL queries over arbitrary datasets — even runtime data.</li><li>Run <em>Live Queries</em> for analytics dashboards, collaborative applications, live feeds, etc.</li><li>Federate across local and remote databases.</li><li>Materialize datasets for offline access.</li><li>Synchronize bidirectionally with arbitrary remote sources.</li></ul><h2 id="overview" tabindex="-1">Overview <a class="header-anchor" href="#overview" aria-label="Permalink to &quot;Overview&quot;">​</a></h2><p>Modern applications need database power without a network layer or the overhead of a physical database server. Sometimes, they also need both — a hybrid model that pairs traditional databases with a local engine. FlashQL addresses just that <strong>in less than <code>80KiB min|zip</code></strong>.</p><p>Just spin up an instance in-app and run SQL:</p><div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre class="shiki material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span style="color:#89DDFF;font-style:italic;">import</span><span style="color:#89DDFF;"> {</span><span style="color:#BABED8;"> FlashClient</span><span style="color:#89DDFF;"> }</span><span style="color:#89DDFF;font-style:italic;"> from</span><span style="color:#89DDFF;"> &#39;</span><span style="color:#C3E88D;">@linked-db/linked-ql/flash</span><span style="color:#89DDFF;">&#39;</span><span style="color:#89DDFF;">;</span></span>
<span class="line"></span>
<span class="line"><span style="color:#C792EA;">const</span><span style="color:#BABED8;"> db </span><span style="color:#89DDFF;">=</span><span style="color:#89DDFF;"> new</span><span style="color:#82AAFF;"> FlashClient</span><span style="color:#BABED8;">()</span><span style="color:#89DDFF;">;</span></span>
...
<span class="line"></span>
<span class="line"><span style="color:#89DDFF;font-style:italic;">await</span><span style="color:#BABED8;"> db</span><span style="color:#89DDFF;">.</span><span style="color:#82AAFF;">disconnect</span><span style="color:#BABED8;">()</span><span style="color:#89DDFF;">;</span></span></code></pre></div><p>From here, you get a robust query engine that can query just as fine from arbtrary data sources as from the local store — a defining feature of <strong>FlashQL</strong>. Meet the FlashQL <strong>Universal I/O</strong> model.</p><h2 id="dialects" tabindex="-1">Dialects <a class="header-anchor" href="#dialects" aria-label="Permalink to &quot;Dialects&quot;">​</a></h2><p>FlashQL supports both <strong>PostgreSQL</strong> and <strong>MySQL</strong> dialects.</p><p>Set globally:</p><div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre class="shiki material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span style="color:#C792EA;">const</span><span style="color:#BABED8;"> db </span><span style="color:#89DDFF;">=</span><span style="color:#89DDFF;"> new</span><span style="color:#82AAFF;"> FlashClient</span><span style="color:#BABED8;">(</span><span style="color:#89DDFF;">{</span><span style="color:#F07178;"> dialect</span><span style="color:#89DDFF;">:</span><span style="color:#89DDFF;"> &#39;</span><span style="color:#C3E88D;">postgres</span><span style="color:#89DDFF;">&#39;</span><span style="color:#89DDFF;"> }</span><span style="color:#BABED8;">)</span><span style="color:#89DDFF;">;</span></span></code></pre></div><p>Optionally specify per query:</p><div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre class="shiki material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span style="color:#89DDFF;font-style:italic;">await</span><span style="color:#BABED8;"> db</span><span style="color:#89DDFF;">.</span><span style="color:#82AAFF;">query</span><span style="color:#BABED8;">(</span><span style="color:#89DDFF;">&#39;</span><span style="color:#C3E88D;">SELECT \`name\` FROM \`users\`</span><span style="color:#89DDFF;">&#39;</span><span style="color:#89DDFF;">,</span><span style="color:#89DDFF;"> {</span><span style="color:#F07178;"> dialect</span><span style="color:#89DDFF;">:</span><span style="color:#89DDFF;"> &#39;</span><span style="color:#C3E88D;">mysql</span><span style="color:#89DDFF;">&#39;</span><span style="color:#89DDFF;"> }</span><span style="color:#BABED8;">)</span><span style="color:#89DDFF;">;</span></span></code></pre></div><p>Where not specified at any scope, FlashQL&#39;s dialect defaults to <code>postgres</code>.</p><h2 id="compatibility" tabindex="-1">Compatibility <a class="header-anchor" href="#compatibility" aria-label="Permalink to &quot;Compatibility&quot;">​</a></h2><p>FlashQL speaks real SQL — in native dialects, but isn&#39;t a full clone of PostgreSQL or MySQL. The goal isn’t to reproduce the entire database engine surface, but to cover the full scope of application-level SQL — streamlined to the declarative and structural features actually used in code: queries, mutations, definitions, and expressions.</p><p>You&#39;d find FlashQL&#39;s implementation coverage of SQL in the <a href="./flashql/lang">Language Reference</a>. (Treat as a live reference.)</p><p>To give a <strong>general</strong> sense of FlashQL’s SQL feature coverage, here are two advanced example queries using PostgreSQL-specific syntax.</p><hr><details><summary>Query 1: (click to show)</summary><div class="language-js"><button title="Copy Code" class="copy"></button><span class="lang">js</span><pre class="shiki material-theme-palenight vp-code" tabindex="0"><code><span class="line"><span style="color:#C792EA;">const</span><span style="color:#89DDFF;"> {</span><span style="color:#BABED8;"> rows </span><span style="color:#89DDFF;">}</span><span style="color:#89DDFF;"> =</span><span style="color:#89DDFF;font-style:italic;"> await</span><span style="color:#BABED8;"> db</span><span style="color:#89DDFF;">.</span><span style="color:#82AAFF;">query</span><span style="color:#BABED8;">(</span></span>
