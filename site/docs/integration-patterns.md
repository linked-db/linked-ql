...
- **`EdgeClient` ↔ `EdgeWorker`**  
  These represent two ends of a transport protocol. `EdgeClient` is the application-facing side of the boundary, and `EdgeWorker` is the runtime side that exposes a database over that boundary. (Full details in [The Edge Family](/docs/setup#the-edge-family)).
...
Should you be new to the overall system, it helps to read this page alongside [What Is LinkedQL](/overview), [Capabilities](/capabilities), and [Setup Guide](/docs/setup).
...
The [EdgeClient](/docs/setup#edgeclient) and [EdgeWorker](/docs/setup#edgeworker) pair gives you the full LinkedQL contract across the boundary.
...
For these capabilities, see the [Appendix A – Handling Protocol Calls](#appendix-a--handling-protocol-calls) section for how backend capabilities map to protocol support.
...
This pattern composes the [Edge transport layer](/docs/setup#edgeclient) with [FlashQL](/flashql) and its [sync model](/flashql/federation-and-sync).
...
That usually fits applications where isolation is preferred and cross-tab sharing is less important than keeping the UI thread free while still preserving the same [EdgeClient](/docs/setup#edgeclient) contract in the UI.
...
* **Pattern A** if you only need remote access and want [EdgeClient](/docs/setup#edgeclient) to preserve the app-facing contract
