...
export class StorageEngine extends SimpleEmitter {
    ...
    async fetch(tableName, keyOrRecord, schemaName = this.#defaultSchemaName, { idxName = null } = {}) {
        const tableStorage = await this.tableStorage(tableName, schemaName);
    ...
    async keys(tableName, schemaName = this.#defaultSchemaName, { idxName = null } = {}) {
        const tableStorage = await this.tableStorage(tableName, schemaName);
        return tableStorage.size;
    }

    async count(tableName, schemaName = this.#defaultSchemaName) {
        const tableStorage = await this.tableStorage(tableName, schemaName);
        return tableStorage.size;
    }
