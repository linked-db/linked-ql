...
$describe('DeSugaring - Expr DeSugaring', () => {
    $describe('LQObjectLiteral', () => {
        $it('should parse an "LQObjectLiteral" expr with formal fields and desugar it - Postgres', async () => {
            ...
            await testParseAndStringify('LQObjectLiteral', [inputSql, outputSql], { deSugar: 2, dialect: 'postgres' });
        });
        ...
        $it('should parse an "LQObjectLiteral" expr with formal fields and desugar it - MySQL', async () => {
            ...
            await testParseAndStringify('LQObjectLiteral', [inputSql, outputSql], { deSugar: 2, dialect: 'mysql' });
        });
    ...
    $describe('LQArrayLiteral', () => {
        $it('should parse an "LQArrayLiteral" expr and desugar it - Postgres', async () => {
            ...
            await testParseAndStringify('LQArrayLiteral', [inputSql, outputSql], { deSugar: 2, dialect: 'postgres' });
        });

        $it('should parse an "LQArrayLiteral" expr and desugar it - MySQL', async () => {
            const inputSql = `['value1', 'value2']`;
            const outputSql = `JSON_ARRAY('value1', 'value2')`;
            await testParseAndStringify('LQArrayLiteral', [inputSql, outputSql], { deSugar: 2, dialect: 'mysql' });
        });
    });
    ...
    $describe('SelectElement', () => {
        ...
        $it('should parse an "SelectElement" expr with an aggregation syntax - Postgres', async () => {
            ...
            await testParseAndStringify('BasicSelectStmt', [inputSql, outputSql], { deSugar: 2, dialect: 'postgres' }, linkedDb);
        });
        ...
        $it('should parse an "SelectElement" expr with an aggregation syntax - MySQL', async () => {
            ...
            await testParseAndStringify('BasicSelectStmt', [inputSql, outputSql], { deSugar: 2, dialect: 'mysql' }, linkedDb);
        });
    ...
    $describe('VersionSpec', () => {
        $it('should parse an "SchemaRef" with version spec to just the SchemaRef - Postgres', async () => {
            ...
            await testParseAndStringify('SchemaRef', [inputSql, outputSql], { deSugar: 2 });
        });

        $it('should parse an "TableRef1" with version spec to just the TableRef1 - Postgres', async () => {
            const inputSql = `tbl@3_3`;
            const outputSql = `tbl`;
            await testParseAndStringify('TableRef1', [inputSql, outputSql], { deSugar: 2 });
        });
    });
    ...
    $describe('LQObjectLiteral As Root Select List', () => {
        $it('should parse an "SelectStmt" with un-aliased LQObjectLiteral as root select list - Postgres', async () => {
            ...
            await testParseAndStringify('SelectStmt', [inputSql, outputSql], { deSugar: 2, dialect: 'postgres' }, linkedDb);
        });
        ...
        $it('should parse an "SelectStmt" with un-aliased LQObjectLiteral as root select list - MySQL', async () => {
            ...
            await testParseAndStringify('SelectStmt', [inputSql, outputSql], { deSugar: 2, dialect: 'mysql' }, linkedDb);
        });
    });
});
...
$describe('DeSugaring - Refs Resolution', () => {
    ...
    $describe('TableRef1', () => {
        $it('should parse a bare "TableRef1" to a fully-qualified TableRef1', async () => {
            ...
            await testParseAndStringify('TableRef1', [inputSql, outputSql], { deSugar: 2 }, linkedDb);
        });
    });
...
FROM public.users AS users`;
            await testParseAndStringify('BasicSelectStmt', [inputSql, outputSql], { deSugar: 3, prettyPrint: true }, linkedDb);
        });
    });
});
...
FROM orders AS o`;
            ...
            await testParseAndStringify('BasicSelectStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
CROSS JOIN (
  ...
  FROM users
            ...
            const outputSql = `SELECT o.id AS id, "$join~0"."$ref~0" AS outerParentEmail, u.innerParentEmail AS innerParentEmail FROM public.orders AS o CROSS JOIN (SELECT users.parent_user AS parent, "$join~0"."$ref~0" AS innerParentEmail FROM public.users LEFT JOIN (SELECT users.id AS "$key~0", users.email AS "$ref~0" FROM public.users) AS "$join~0" ON users.parent_user = "$join~0"."$key~0") AS u LEFT JOIN (SELECT users.id AS "$key~0", users.email AS "$ref~0" FROM public.users) AS "$join~0" ON u.parent = "$join~0"."$key~0"`;
            await testParseAndStringify('BasicSelectStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
FROM orders AS o`;
            const outputSql = `SELECT o.id AS id, "$join~0"."$ref~0" AS status, "$join~0"."$ref~1" AS status FROM public.orders AS o LEFT JOIN (SELECT "$join~0"."$ref~0" AS "$key~0", orders.status AS "$ref~0", orders.status AS "$ref~1" FROM public.orders LEFT JOIN (SELECT orders.id AS "$key~0", orders.parent_order AS "$ref~0" FROM public.orders) AS "$join~0" ON orders.parent_order = "$join~0"."$key~0") AS "$join~0" ON o.id = "$join~0"."$key~0"`;
            await testParseAndStringify('BasicSelectStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
VALUES
  ...
  ('dffff', ROW (5, 2000))`;
            const outputSql =
                `WITH "$main~0" AS (INSERT INTO public.users (email) VALUES ROW ('dd'), ROW ('dffff') RETURNING users.id), "$main~0~indices" AS (SELECT "$main~0".id AS id, ROW_NUMBER() OVER () AS "$row_number~b" FROM "$main~0"), "$dependency~0" AS (INSERT INTO public.users (parent_user) SELECT "$main~0~indices".id FROM "$main~0~indices" WHERE "$main~0~indices"."$row_number~b" = 1 RETURNING users.id), "$dependency~1" AS (INSERT INTO public.users (parent_user) SELECT "$main~0~indices".id FROM "$main~0~indices" WHERE "$main~0~indices"."$row_number~b" = 2 RETURNING users.id), "$dependent~0" AS (INSERT INTO public.users (id, email, parent_user) VALUES ROW (50, 2100, (SELECT "$dependency~0".id FROM "$dependency~0")), ROW (5, 2000, (SELECT "$dependency~1".id FROM "$dependency~1"))) SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('InsertStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
DEFAULT VALUES`;
            const outputSql =
                `WITH "$dependency~0" AS (INSERT INTO public.users (username, email) DEFAULT VALUES RETURNING users.id) INSERT INTO public.users (email, parent_user) VALUES ROW (DEFAULT, (SELECT "$dependency~0".id FROM "$dependency~0"))`;
            await testParseAndStringify('InsertStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
DEFAULT VALUES`;
            const outputSql =
                `WITH "$dependency~1" AS (INSERT INTO public.users (username, email) DEFAULT VALUES RETURNING users.id), "$dependency~0" AS (INSERT INTO public.users (parent_user) VALUES ROW ((SELECT "$dependency~1".id FROM "$dependency~1")) RETURNING users.id) INSERT INTO public.users (email, parent_user) VALUES ROW (DEFAULT, (SELECT "$dependency~0".id FROM "$dependency~0"))`;
            await testParseAndStringify('InsertStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
DEFAULT VALUES`;
            const outputSql =
                `WITH "$main~0" AS (INSERT INTO public.users (email) DEFAULT VALUES RETURNING users.id), "$dependent~0" AS (INSERT INTO public.users (username, email, parent_user) VALUES ROW (DEFAULT, DEFAULT, (SELECT "$main~0".id FROM "$main~0"))) SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('InsertStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
SELECT id, order_total AS email, parent_order AS rr FROM orders as t WHERE 1`;
            const outputSql =
                `WITH "$memo~0" AS (SELECT ROW_NUMBER() OVER () AS "$row_number~a", t.id AS id, t.order_total AS email, t.parent_order AS rr FROM public.orders AS t WHERE 1), "$dependency~1" AS (INSERT INTO public.users (email) SELECT "$memo~0".rr AS email FROM "$memo~0" RETURNING users.id), "$dependency~1~indices" AS (SELECT "$dependency~1".id AS id, ROW_NUMBER() OVER () AS "$row_number~b" FROM "$dependency~1"), "$dependency~0" AS (INSERT INTO public.users (parent_user) SELECT (SELECT "$dependency~1~indices".id FROM "$dependency~1~indices" WHERE "$dependency~1~indices"."$row_number~b" = "$memo~0"."$row_number~a") AS parent_user FROM "$memo~0" RETURNING users.id), "$dependency~0~indices" AS (SELECT "$dependency~0".id AS id, ROW_NUMBER() OVER () AS "$row_number~b" FROM "$dependency~0") INSERT INTO public.users (id, email, parent_user) SELECT "$memo~0".id AS id, "$memo~0".email AS email, (SELECT "$dependency~0~indices".id FROM "$dependency~0~indices" WHERE "$dependency~0~indices"."$row_number~b" = "$memo~0"."$row_number~a") AS parent_user FROM "$memo~0"`;
            await testParseAndStringify('InsertStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
SELECT id, order_total AS email, parent_order AS rr FROM orders as t WHERE 1`;
            const outputSql =
                `WITH "$memo~0" AS (SELECT ROW_NUMBER() OVER () AS "$row_number~a", t.id AS id, t.order_total AS email, t.parent_order AS rr FROM public.orders AS t WHERE 1), "$main~0" AS (INSERT INTO public.users (id, email) SELECT "$memo~0".id AS id, "$memo~0".email AS email FROM "$memo~0" RETURNING users.id), "$main~0~indices" AS (SELECT "$main~0".id AS id, ROW_NUMBER() OVER () AS "$row_number~b" FROM "$main~0"), "$dependent~0" AS (INSERT INTO public.users (email, parent_user) SELECT "$memo~0".rr AS email, (SELECT "$main~0~indices".id FROM "$main~0~indices" WHERE "$main~0~indices"."$row_number~b" = "$memo~0"."$row_number~a") AS parent_user FROM "$memo~0") SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('InsertStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
SELECT id, order_total AS email, parent_order AS rr FROM orders as t WHERE 1`;
            const outputSql =
                `WITH "$memo~0" AS (SELECT ROW_NUMBER() OVER () AS "$row_number~a", t.id AS id, t.order_total AS email, t.parent_order AS rr FROM public.orders AS t WHERE 1), "$main~0" AS (INSERT INTO public.users (id, email) SELECT "$memo~0".id AS id, "$memo~0".email AS email FROM "$memo~0" RETURNING users.id), "$main~0~indices" AS (SELECT "$main~0".id AS id, ROW_NUMBER() OVER () AS "$row_number~b" FROM "$main~0"), "$dependency~0" AS (INSERT INTO public.users (parent_user) SELECT (SELECT "$main~0~indices".id FROM "$main~0~indices" WHERE "$main~0~indices"."$row_number~b" = "$memo~0"."$row_number~a") AS parent_user FROM "$memo~0" RETURNING users.id), "$dependency~0~indices" AS (SELECT "$dependency~0".id AS id, ROW_NUMBER() OVER () AS "$row_number~b" FROM "$dependency~0"), "$dependent~0" AS (INSERT INTO public.users (email, parent_user) SELECT "$memo~0".rr AS email, (SELECT "$dependency~0~indices".id FROM "$dependency~0~indices" WHERE "$dependency~0~indices"."$row_number~b" = "$memo~0"."$row_number~a") AS parent_user FROM "$memo~0") SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('InsertStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
SELECT id, order_total AS email, ROW(2, parent_order) AS rr FROM orders as t WHERE 1`;
            const outputSql =
                `WITH "$memo~0" AS (SELECT ROW_NUMBER() OVER () AS "$row_number~a", t.id AS id, t.order_total AS email, 2 AS "rr~rand~0", t.parent_order AS "rr~rand~1" FROM public.orders AS t WHERE 1), "$main~0" AS (INSERT INTO public.users (username, email) SELECT "$memo~0".id AS id, "$memo~0".email AS email FROM "$memo~0" RETURNING users.id), "$main~0~indices" AS (SELECT "$main~0".id AS id, ROW_NUMBER() OVER () AS "$row_number~b" FROM "$main~0"), "$dependency~0" AS (INSERT INTO public.users (status, email) SELECT "$memo~0"."rr~rand~0" AS status, "$memo~0"."rr~rand~1" AS email FROM "$memo~0" RETURNING users.id), "$dependency~0~indices" AS (SELECT "$dependency~0".id AS id, ROW_NUMBER() OVER () AS "$row_number~b" FROM "$dependency~0"), "$dependency~1" AS (INSERT INTO public.users (parent_user) SELECT (SELECT "$main~0~indices".id FROM "$main~0~indices" WHERE "$main~0~indices"."$row_number~b" = "$memo~0"."$row_number~a") AS parent_user FROM "$memo~0" RETURNING users.id), "$dependency~1~indices" AS (SELECT "$dependency~1".id AS id, ROW_NUMBER() OVER () AS "$row_number~b" FROM "$dependency~1"), "$dependent~0" AS (INSERT INTO public.users (parent_user, parent_user) SELECT (SELECT "$dependency~0~indices".id FROM "$dependency~0~indices" WHERE "$dependency~0~indices"."$row_number~b" = "$memo~0"."$row_number~a") AS parent_user, (SELECT "$dependency~1~indices".id FROM "$dependency~1~indices" WHERE "$dependency~1~indices"."$row_number~b" = "$memo~0"."$row_number~a") AS parent_user FROM "$memo~0") SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('InsertStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
    });
});
...
SET
  ...
  (username, parent_user ~> id) = (232, 3445)`;
            const outputSql =
                `WITH "$main~0" AS (UPDATE public.users AS u SET id = 2, email = 'x@x.com', (username, parent_user) = ROW (232, u.parent_user) RETURNING u.parent_user), "$dependency~0" AS (UPDATE public.users AS users SET (id) = ROW (3445) WHERE users.id IN (SELECT "$main~0".parent_user FROM "$main~0")) SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('UpdateStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
SET
  ...
  (username, parent_user ~> parent_user ~> id) = (232, 3445)`;
            const outputSql =
                `WITH "$main~0" AS (UPDATE public.users AS u SET id = 2, email = 'x@x.com', (username, parent_user) = ROW (232, u.parent_user) RETURNING u.parent_user), "$main~1" AS (UPDATE public.users AS users SET (parent_user) = ROW (users.parent_user) WHERE users.id IN (SELECT "$main~0".parent_user FROM "$main~0") RETURNING users.parent_user), "$dependency~1" AS (UPDATE public.users AS users SET (id) = ROW (3445) WHERE users.id IN (SELECT "$main~1".parent_user FROM "$main~1")) SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('UpdateStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
SET
  ...
  (username, (parent_user <~ users) ~> id) = (232, 3445)`;
            const outputSql =
                `WITH "$main~0" AS (UPDATE public.users AS u SET id = 2, email = 'x@x.com', (username) = ROW (232) RETURNING u.id), "$dependent~0" AS (UPDATE public.users AS users SET (id) = ROW (3445) WHERE users.parent_user IN (SELECT "$main~0".id FROM "$main~0")) SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('UpdateStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
SET
  ...
  (username, (parent_user <~ users) ~> parent_user ~> id) = (232, 3445)`;
            const outputSql =
                `WITH "$main~0" AS (UPDATE public.users AS u SET id = 2, email = 'x@x.com', (username) = ROW (232) RETURNING u.id), "$main~1" AS (UPDATE public.users AS users SET (parent_user) = ROW (users.parent_user) WHERE users.parent_user IN (SELECT "$main~0".id FROM "$main~0") RETURNING users.parent_user), "$dependency~0" AS (UPDATE public.users AS users SET (id) = ROW (3445) WHERE users.id IN (SELECT "$main~1".parent_user FROM "$main~1")) SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('UpdateStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
SET
  ...
  (username, (parent_user <~ parent_user <~ users) ~> parent_user ~> id) = (222, 3445)`;
            const outputSql =
                `WITH "$main~0" AS (UPDATE public.users AS u SET id = 2, email = 'x@x.com', (username) = ROW (222) RETURNING u.id), "$main~1" AS (UPDATE public.users AS users SET (parent_user) = ROW (users.parent_user) FROM (SELECT "$join~1:users".id AS id, "$join~1:users".parent_user AS parent_user FROM public.users AS "$join~1:users" WHERE users.id = "$join~1:users".id) AS "$join~1:users" LEFT JOIN (SELECT users.id AS "$key~0", users.parent_user AS "$ref~0" FROM public.users) AS "$join~0" ON "$join~1:users".parent_user = "$join~0"."$key~0" WHERE "$join~0"."$ref~0" IN (SELECT "$main~0".id FROM "$main~0") RETURNING users.parent_user), "$dependency~0" AS (UPDATE public.users AS users SET (id) = ROW (3445) WHERE users.id IN (SELECT "$main~1".parent_user FROM "$main~1")) SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('UpdateStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
    });
...
WHERE parent_user ~> id = 2`;
            const outputSql =
                `WITH "$main~0" AS (UPDATE public.users AS u SET id = 2, email = 'x@x.com', (username, parent_user) = ROW (232, u.parent_user) FROM (SELECT "$join~1:u".id AS id, "$join~1:u".parent_user AS parent_user FROM public.users AS "$join~1:u" WHERE u.id = "$join~1:u".id) AS "$join~1:u" LEFT JOIN (SELECT users.id AS "$key~0", users.id AS "$ref~0" FROM public.users) AS "$join~0" ON "$join~1:u".parent_user = "$join~0"."$key~0" WHERE "$join~0"."$ref~0" = 2 RETURNING u.parent_user), "$dependency~0" AS (UPDATE public.users AS users SET (id) = ROW (3445) WHERE users.id IN (SELECT "$main~0".parent_user FROM "$main~0")) SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('UpdateStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
WHERE parent_user ~> id = 2`;
            const outputSql =
                `WITH "$main~0" AS (UPDATE public.users AS u SET id = 2, email = 'x@x.com', (username, parent_user) = ROW (232, u.parent_user) FROM (SELECT "$join~1:u".id AS id, "$join~1:u".parent_user AS parent_user FROM public.users AS "$join~1:u" WHERE u.id = "$join~1:u".id) AS "$join~1:u" LEFT JOIN (SELECT users.id AS "$key~0", users.id AS "$ref~0" FROM public.users) AS "$join~0" ON "$join~1:u".parent_user = "$join~0"."$key~0" WHERE "$join~0"."$ref~0" = 2 RETURNING u.parent_user), "$main~1" AS (UPDATE public.users AS users SET (parent_user) = ROW (users.parent_user) WHERE users.id IN (SELECT "$main~0".parent_user FROM "$main~0") RETURNING users.parent_user), "$dependency~1" AS (UPDATE public.users AS users SET (id) = ROW (3445) WHERE users.id IN (SELECT "$main~1".parent_user FROM "$main~1")) SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('UpdateStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
WHERE parent_user ~> id = 2`;
            const outputSql =
                `WITH "$main~0" AS (UPDATE public.users AS u SET id = 2, email = 'x@x.com', (username) = ROW (232) FROM (SELECT "$join~1:u".id AS id, "$join~1:u".parent_user AS parent_user FROM public.users AS "$join~1:u" WHERE u.id = "$join~1:u".id) AS "$join~1:u" LEFT JOIN (SELECT users.id AS "$key~0", users.id AS "$ref~0" FROM public.users) AS "$join~0" ON "$join~1:u".parent_user = "$join~0"."$key~0" WHERE "$join~0"."$ref~0" = 2 RETURNING u.id), "$dependent~0" AS (UPDATE public.users AS users SET (id) = ROW (3445) WHERE users.parent_user IN (SELECT "$main~0".id FROM "$main~0")) SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('UpdateStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
WHERE parent_user ~> id = 2`;
            const outputSql =
                `WITH "$main~0" AS (UPDATE public.users AS u SET id = 2, email = 'x@x.com', (username) = ROW (232) FROM (SELECT "$join~1:u".id AS id, "$join~1:u".parent_user AS parent_user FROM public.users AS "$join~1:u" WHERE u.id = "$join~1:u".id) AS "$join~1:u" LEFT JOIN (SELECT users.id AS "$key~0", users.id AS "$ref~0" FROM public.users) AS "$join~0" ON "$join~1:u".parent_user = "$join~0"."$key~0" WHERE "$join~0"."$ref~0" = 2 RETURNING u.id), "$main~1" AS (UPDATE public.users AS users SET (parent_user) = ROW (users.parent_user) WHERE users.parent_user IN (SELECT "$main~0".id FROM "$main~0") RETURNING users.parent_user), "$dependency~0" AS (UPDATE public.users AS users SET (id) = ROW (3445) WHERE users.id IN (SELECT "$main~1".parent_user FROM "$main~1")) SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('UpdateStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
WHERE parent_user ~> id = 2`;
            const outputSql =
                `WITH "$main~0" AS (UPDATE public.users AS u SET id = 2, email = 'x@x.com', (username) = ROW (222) FROM (SELECT "$join~1:u".id AS id, "$join~1:u".parent_user AS parent_user FROM public.users AS "$join~1:u" WHERE u.id = "$join~1:u".id) AS "$join~1:u" LEFT JOIN (SELECT users.id AS "$key~0", users.id AS "$ref~0" FROM public.users) AS "$join~0" ON "$join~1:u".parent_user = "$join~0"."$key~0" WHERE "$join~0"."$ref~0" = 2 RETURNING u.id), "$main~1" AS (UPDATE public.users AS users SET (parent_user) = ROW (users.parent_user) FROM (SELECT "$join~1:users".id AS id, "$join~1:users".parent_user AS parent_user FROM public.users AS "$join~1:users" WHERE users.id = "$join~1:users".id) AS "$join~1:users" LEFT JOIN (SELECT users.id AS "$key~0", users.parent_user AS "$ref~0" FROM public.users) AS "$join~0" ON "$join~1:users".parent_user = "$join~0"."$key~0" WHERE "$join~0"."$ref~0" IN (SELECT "$main~0".id FROM "$main~0") RETURNING users.parent_user), "$dependency~0" AS (UPDATE public.users AS users SET (id) = ROW (3445) WHERE users.id IN (SELECT "$main~1".parent_user FROM "$main~1")) SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('UpdateStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
    });
...
SET
  ...
  (username, parent_user ~> (id, username)) = (SELECT 232, (3445, 333) FROM orders)`;
            const outputSql =
                `WITH "$memo~0" AS (SELECT ROW_NUMBER() OVER () AS "$row_number~a", 232 AS username, 3445 AS "$value~0", 333 AS "$value~1" FROM public.orders), "$main~0" AS (UPDATE public.users AS u SET id = 2, email = 'x@x.com', (username, parent_user) = (SELECT "$memo~0".username AS username, u.parent_user AS parent_user FROM "$memo~0") RETURNING u.parent_user), "$dependency~0" AS (UPDATE public.users AS users SET (id, username) = (SELECT "$memo~0"."$value~0" AS "$value~0", "$memo~0"."$value~1" AS "$value~1" FROM "$memo~0") WHERE users.id IN (SELECT "$main~0".parent_user FROM "$main~0")) SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('UpdateStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
...
SET
  ...
  (username, parent_user ~> parent_user ~> (id, username)) = (232, (SELECT 3445, 333 FROM orders))`;
            const outputSql =
                `WITH "$main~0" AS (UPDATE public.users AS u SET id = 2, email = 'x@x.com', (username, parent_user) = ROW (232, u.parent_user) RETURNING u.parent_user), "$main~1" AS (UPDATE public.users AS users SET (parent_user) = ROW (users.parent_user) WHERE users.id IN (SELECT "$main~0".parent_user FROM "$main~0") RETURNING users.parent_user), "$dependency~1" AS (UPDATE public.users AS users SET (id, username) = (SELECT 3445, 333 FROM public.orders) WHERE users.id IN (SELECT "$main~1".parent_user FROM "$main~1")) SELECT COUNT(*) AS COUNT FROM "$main~0"`;
            await testParseAndStringify('UpdateStmt', [inputSql, outputSql], { deSugar: 2, prettyPrint: true }, linkedDb);
        });
