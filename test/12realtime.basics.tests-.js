...
describe('Local - Subscriptions', () => {
    ...
    describe('LiteQL - Subscriptions', () => {
        ...
        before(async () => {
            ...
            relation = { namespace: 'lq_test_public', name: 'tbl1', keyColumns: ['id'] };
            eventsCollector = collectEvents();
...
describe('Classic - Subscriptions', () => {
    ...
    describe('PGDriver - Subscriptions (2)', () => {
        ...
        const namespaceNames = ['lq_test_public', 'lq_test_private'];
        ...
        before(async () => {
            ...
            for (const namespaceName of namespaceNames) {
                await driver.dropSchema(namespaceName);
                await driver.createSchema(namespaceName);
                await driver.query(`CREATE TABLE IF NOT EXISTS ${namespaceName}.tbl1 (
                    id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
                    fname TEXT,
                    email TEXT
                )`);
                await driver.query(`CREATE TABLE IF NOT EXISTS ${namespaceName}.tbl2 (
                    id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
                    fname TEXT,
                    email TEXT
                )`);
                relations.push({ namespace: namespaceName, name: 'tbl1', keyColumns: ['id'] });
            }
        });
        ...
        it('should emit mutation event ("insert") for an insert operation', async () => {
            ...
            driver.subscribe({ [namespaceNames[0]]: ['*'] }, eventsCollector3.listener);

            const eventsCollector4 = collectEvents();
            driver.subscribe({ [namespaceNames[1]]: '*' }, eventsCollector4.listener);

            const eventsCollector5 = collectEvents();
            driver.subscribe({ [namespaceNames[1]]: 'tbl1' }, eventsCollector5.listener);
            ...
            await driver.query(`
                INSERT INTO ${namespaceNames[0]}.tbl1 (id, fname, email)
                VALUES
                    (${records[0].id}, '${records[0].fname}', '${records[0].email}'),
                    (${records[1].id}, '${records[1].fname}', '${records[1].email}');
                -- ||||||||||||||||||||
                INSERT INTO ${namespaceNames[1]}.tbl1 (id, fname, email)
                VALUES
        ...
        it('should emit mutation event ("update") for an update operation', async () => {
            ...
            await driver.query(`
                UPDATE ${namespaceNames[0]}.tbl1 SET id = id + 1 WHERE id IN (${keys1.map((r) => r.id).join(', ')});
                -- ||||||||||||||||||||
                DELETE FROM ${namespaceNames[1]}.tbl1 WHERE id IN (${keys2.map((r) => r.id).join(', ')});
            `);
        ...
        describe('- QueryWindow', () => {
            ...
            it('should do basic reactivity for a basic single-table query', async () => {
                ...
                await schemaInference.provide([{ namespace: 'lq_test_public', tables: ['tbl1'] }]);
            ...
            it('should do basic reactivity for a basic two-table query', async () => {
                ...
                await schemaInference.provide([{ namespace: 'lq_test_private', tables: ['tbl1', 'tbl2'] }]);
